...
void CPathPlanDlg::OnBnClickedButtonJoystick()
{
	...
	{
		result=joySetCapture(CDialog::m_hWnd, JOYSTICKID1, 0, FALSE);   //设置摇杆函数，将返回值赋值给result---乐
		
		AllocConsole();
		_cprintf("检测调试""ButtonJoystickFlag=%d",ButtonJoystickFlag);

		MessageBox("实验重新获取游戏杆");
	...
	_cprintf("检测调试222""ButtonJoystickFlag=%d",ButtonJoystickFlag);
...
///////////////////////////////////////添加任务区域代码---乐/////////////////////////////////////////////////////
	case MM_JOY1MOVE : 
		test(message, wParam, lParam, 9);     //测试传递的信息及参数---乐---自己编写
		if (flag)
		{
			for (int i=0; i <= 10; ++i)
			{
				...
				if (wParam ==btn)
				{
					MessageBeep(MB_ICONEXCLAMATION); //播放系统声效---乐
					switch (i)
					{
					case 0:
						m_JNT0+=vel_JoyStick;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		WORD x, y;                                                 //无符号型16位整数，2byte，取值范围0--65535----乐
		POINT pt;                                                 //定义一个存放鼠标坐标值得结构体变量，POINT结构体在windef.h头文件中定义---乐
//		UpdateData(TRUE);
		//获取当前屏幕光标位置
		GetCursorPos(&pt);                                        //GetCursorPos()函数需要一个结构体指针，存放获取的左边点数值，x,y---乐
...
void CPathPlanDlg::OnDestroy() 
{
	...
	joyReleaseCapture(JOYSTICKID1);                                     //释放获取的摇杆1---乐
