...
void CRobotControlView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	CRobotControlDoc* pDoc = GetDocument()->doc_delay;
	if (m_LButtonDown)
	{
		CSize rotation = point- m_LDownPos ;
		m_LDownPos = point;
		pDoc->m_zRotation += rotation.cx/3; //x坐标增大,绕z轴逆时针转动
		pDoc->m_xRotation += rotation.cy/3; //y坐标增大,绕x	
	
		//AllocConsole();
	    //_cprintf("test the fundation !%f",pDoc->m_xRotation );
	...
	if (m_RButtonDown)   //---乐---图形的缩放函数
	{
		...
		pDoc->m_zoom -=(GLfloat)zoom.cx/50;  //x坐标增大,zoom越小,-zoom(不是绝对值)越大,放大
		pDoc->m_zoom +=(GLfloat)zoom.cy/50;  //y坐标越大,zoom越大,-zoom越小,缩小
		if (pDoc->m_zoom>100) pDoc->m_zoom=100;
		if (pDoc->m_zoom<0.1) pDoc->m_zoom=0.1;
		InvalidateRect(NULL, FALSE);
	}
